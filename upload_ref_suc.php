		...
		for($i=0;$i<$totalSheet;$i++){
			...
			foreach ($Reader as $Row)
	        {
				...
				$comp_date= date("Y-m-d", strtotime($comp_date1));
				 ...
				 $q10="select * from refferences where location='$lname' and indus_id='$indus_id' and req_ref='$req_ref' ";
				
				$p=mysqli_query($link,$q10) or die(mysqli_error($link));
				//$p=$link->query($q);
				
				$r=mysqli_fetch_array($p);
				$req_ref1=addslashes($r['req_ref']);
				$site_name1=$r['site_name'];
