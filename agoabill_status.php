...
// $u1=($u);
 ...
 $res=mysqli_query($link,$query) or die(mysqli_error($link));
	...
	print "self.location='edit_goabill1.php?id=$id'";
	print "</script>";
	...
	if($res){
		...
		if($bar_code!=''){
		$v="update `add_goabill1` set `date`='$date', `service_no`='$service_no', `po_no`='$po_no', `po_date`='$po_date', `site_name`='$site_name',
	...
	`total_cgst`='$totcgst',`total_gst`='$tgsttotal',qr_code='$bar_code',`bill_status`='$status', apuser='$user',bill_submit_date='$sub_date',`payment_doc_date`='$payment_doc_date',`payment_doc_no`='$payment_doc_no',pcw='$pcw',ackno='$ackno',ackdt='$ackdt' where id='$id'";
	...
	`req_ref`='$req_ref', `seeking_id`='$seeking_id', `state`='$state', `seeking_opt`='$seeking_opt', `rfaid_date`='$rfaid_date',
	 `allcoation_date`='$allcoation_date', 
	`wcc_num`='$wcc_num', `wcc_rec_num`='$wcc_rec_num', `total_amnt`='$tot',`total_sgst`='$sgsttotal',
	`total_cgst`='$totcgst',`total_gst`='$tgsttotal',`payment_doc_date`='$payment_doc_date',`payment_doc_no`='$payment_doc_no',`bill_status`='$status', apuser='$user',pcw='$pcw',ackno='$ackno',ackdt='$ackdt' where id='$id'";
		$sq=mysqli_query($link,$v1) or die(mysqli_error($link));
	}
	...
	if($status=='apporved'){
			header('Location:goaosinvoice1.php?id='.$id.'&loc='.$location.'&email='.$email);
