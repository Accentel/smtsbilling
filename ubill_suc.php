...
if(isset($_POST['submit'])){
	...
	if(in_array($_FILES["file"]["type"],$mimes)){
		...
		for($i=0;$i<$totalSheet;$i++){
			...
			//	 $desc = isset($Row[1]) ? addslashes($Row[1]) : '';
				 
			//	 $desc = "";
              //  if(isset($Row[1])) {
              //      $desc = mysqli_real_escape_string($link,$Row[1]);
//}
			...
			//	$amount = isset($Row[5]) ? $Row[5] : '';
                ...
                $description  = "";
                if(isset($Row[1])) {
                    $description  = mysqli_real_escape_string($link,$Row[1]);
                }	
					
					  $uom = "";
                if(isset($Row[2])) {
                    $uom = mysqli_real_escape_string($link,$Row[2]);
                }	
					
					
			  $price = "";
                if(isset($Row[3])) {
                    $price = mysqli_real_escape_string($link,$Row[3]);
                }			
					
					
				  $qty = "";
                if(isset($Row[4])) {
                    $qty = mysqli_real_escape_string($link,$Row[4]);
                }	
                
                  $amount = "";
                if(isset($Row[5])) {
                    $amount = mysqli_real_escape_string($link,$Row[5]);
                }	
