...
<b>Invoice No  :<?php echo $service_no; ?> <br/>Invoice Date:<?php echo $invdate; ?> <br/>STATE       : MAHARASHTRA <br/> STATE CODE  :27 </b>  </th>
<th style="width:50%" align="left">
<b>PAN No:-AAACO8174A <br/>GST NO:-27AAACO8174A1ZL </b></th>
...
<table style="width:100%" border=1 BORDERCOLOR=BLACK>
		 <tr style="width:100%" >
		  <td style="width:60%">Po No &nbsp;&nbsp;&nbsp; <?php echo $po_no; ?></td>
		  <td style="width:40%">PO DATE&nbsp;&nbsp;&nbsp;<?php echo $po_date; ?></td>
		  </tr>
		 <!-- <tr style="width:100%">
		  <td style="width:60%">Old Po No &nbsp;&nbsp;&nbsp; <?php echo $opo_no; ?></td>
		  <td style="width:40%">PO DATE&nbsp;&nbsp;&nbsp;<?php if($opo_date1!='0000-00-00'){
			 echo $opo_date; } else {
			echo "";	 
			 }?></td>
		  </tr>-->
		  <tr style="width:100%">
			...
			WCC RECIEPT NO:  <?php echo $wcc_rec_num; ?>
			</td>
			<td  style="width:40%">
...
<?php
  $y="select distinct(cgst) from add_mbill where service_no='$service_no' and id1='$bid' ";
...
$cont=mysqli_num_rows($sq);
 ...
 while($r=mysqli_fetch_array($sq)){
	 ...
	 $u="select  sum(tax_amnt) as tax_amnt,sum(gst_amnt) as gst from add_mbill where service_no='$service_no' and cgst='$sggst' and id1='$bid'";
...
while($t1=mysqli_fetch_array($t)){
	...
	<td  style="width:50%;"><?php echo '<b>'.$t1['item_desc'].'</b>'; ?></td>
	<th style="width:10%;">
	<?php 
	if($t1['hsnno']!=='0'){
	echo $t1['hsnno'];
	...
	<th style="width:9%;"><?php echo $t1['uom']; ?></th>
	<th style="width:10%;"><?php echo number_format($t1['qty'],4); ?></th>
	<th style="width:10%;"><?php echo $t1['price']; ?></th>
	<th style="width:10%; border-right-color: #000 !important;"><?php echo $tax=number_format((float)$t1['tax_amnt'],1, '.', ''); ?></th>
...
<tr>
 ...
 <th style="font-size:16px; border-right-color: #000 !important;"> <?php echo $tamt=$tax_amnt+$gst; ?></th>
 </tr>
...
$mpdf=new \mPDF('c','A4','','',5,5,50,15,0,0);

$mpdf->SetHTMLHeader('<img src="headerlogo.jpg"/>');
$mpdf->SetHTMLFooter('<div style="text-align:center">Branch Office : A3/302,Kohinoor Estate, Mulla Road, Wakedewadi,Pune-411001.</div>             
    <div style="text-align:center">Email : ospsinfo@ospsindia.com, ospshyd@yaho
Success!o.com, Website:www.ospsindia.com</div>
<div style="text-align:center">CIN:U64203TG2004PTC042772</div>
<div style="text-align:center;font-size:12px;">Regd.Office : Farha Villa, D.No.8-1-22/1/J/1, 7 Tombs Road, Tolichowki, Hyderabad - 500 008. T.S.</div>
');

$mpdf->writeHTML($body);
$mpdf->Output('mdemo.pdf','F');
error_reporting(E_ALL);
ini_set('display_errors', 1);
	print "<script>";
	print "self.location='mdemo.pdf';";
	print "</script>";
