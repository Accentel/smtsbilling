...
$q=mysqli_query($link,"select * from add_goabill1 where id='$id'") or die(mysqli_error($link));
...
$sq=mysqli_query($link,"select distinct(sgst) from add_goabill where service_no='$service_no' ");
 ...
 while($r=mysqli_fetch_array($sq)){
	 ...
	 $sq1=mysqli_query($link,"select  sum(tax_amnt) as tax_amnt,sum(gst_amnt) as gst from add_goabill where service_no='$service_no' and sgst='$sggst'");
 ...
 $aa="select * from add_goabill where service_no='$service_no' and cgst='$sggst' and id1='$id'";
...
$html2pdf->setTestIsImage(false);
		...
		$content = $html2pdf->Output('goainvoice-'.$id.'.pdf','F');
		
		
		
		header('Location:goabill_list.php?id='.$id.'&loc='.$loc.'&email='.$email.'&sno='.$service_no);
