...
if (isset($_POST['submit'])) {
    ...
    $t = mysqli_query($link, "SELECT * FROM employee WHERE emp_name='$ses'") or die(mysqli_error($link));
  ...
  if ($result->num_rows > 0) {
      ...
      $insertStmt = $link->prepare("INSERT INTO add_bill1 (date, service_no, po_no, site_name, district, indus_id, req_ref, seeking_id, state, seeking_opt, wcc_num,  total_amnt, user, type_work, pcw, bill_status, bill_submit_date, mob, proj_name, net_amnt, image_1, image_2, image_3, image_4, pno, ino, inv) 
                                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,? ,? ,? ,?)");
